...
function GroBags() {
  ...
  const { toggleMode, mode, filterType, setFilterType } = context
      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-8 md:py-16 mx-auto ">
          <div class=" w-full mb-6 lg:mb-10">
            <h1 class="sm:text-3xl text-2xl font-bold title-font mb-2 text-gray-900 text-center" style={{ color: mode === 'dark' ? 'white' : '' }}>Planters/GroBags</h1>
          ...
          <div className="flex flex-wrap -m-4 mb-5">
            {product.filter((obj) => obj.title.toLowerCase().includes(searchkey))
              .filter((obj) =>obj.subCategory.includes('groBags'))
                    ...
                    <div className="p-4 md:w-1/3 lg:w-1/4 drop-shadow-lg " key={index}>
                      <div className="h-full border-2 hover:shadow-gray-100 hover:shadow-md transition-shadow duration-300 ease-in-out bg-gray-100    border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                        <div className="flex justify-center cursor-pointer" onClick={() => navigate(`/productinfo/${item.id}`)}>
                          <img className="w-full  hover:scale-110 transition-scale-110  duration-300 ease-in-out" src={item.products.imageUrl} alt="blog" />
                        </div>
                        <div className="lg:p-5 md:p-3 border-t-2">
                          <h2 className="tracking-widest text-xs title-font font-medium text-gray-400 mb-1" style={{ color: mode === 'dark' ? 'white' : '', }}>Paudha</h2>
                          <h1 className="title-font text-lg font-medium text-gray-900 mb-2" style={{ color: mode === 'dark' ? 'white' : '', }}>{item.products.title.substr(0, 50)}</h1>
                          ...
                          <p className="leading-relaxed mb-3" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{item.products.price}</p>

                          <div className=" flex justify-between   lg:gap-2">
                            <div className=" bg-gray-300 p-2.5 md:px-2 lg:px-2.5 rounded-full cursor-pointer" onClick={() => addToCart(item)}>
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-black">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                              </svg>
                            </div>
                            <div className="bg-gray-300 p-2.5 md:px-2 lg:px-2.5 rounded-full cursor-pointer">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-black">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                              </svg>
                            </div>
                            <div className="bg-gray-300 p-2.5 md:px-2 lg:px-2.5 rounded-full cursor-pointer" onClick={() => navigate(`/productinfo/${item.id}`)}>
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-black">
